...
const renderClient = row => {
  const stateNum = Math.floor(Math.random() * 6),
    ...
    return <Avatar color={color || 'primary'} className='me-1' content={row.person_id || 'John Doe'} initials />
...
export const columns = [
    ...
    cell: row => (
      <div className='d-flex justify-content-left align-items-center'>
        ...
        <div className='d-flex flex-column'>
          <Link
            to={`/Prescription/order/${row.id}`}
            className='user_name text-truncate text-body'
            // onClick={() => store.dispatch(getPrescription(row.id))}
          >
  ...
  },
  {
    ...
    cell: row => <span className='text-capitalize'>{row.first_name}</span>
  },
  {
    name: 'gender',
    sortable: true,
  ...
  },
  {
    ...
    cell: row => <span className='text-capitalize'>{row.updated_at}</span>
  },
  {
    name: 'Actions',
    minWidth: '100px',
